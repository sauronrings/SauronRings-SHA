...
// Copyright (c) 2013 SauronRings developers
...
const string strMessageMagic = "SauronRings Signed Message:\n";
...
static const int64 nTargetTimespan = 45 * 90; // 90 blocks (67.5 minutes)
static const int64 nTargetSpacing = 45; // 45 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 90 blocks

static const int64 nAveragingInterval = nInterval * 5; // 450 blocks
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; // 40 minutes

static const int64 nMaxAdjustDown = 35; // 35% adjustment down
static const int64 nMaxAdjustUp = 35; // 35% adjustment up
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SauronRingsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SauronRingsMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("SauronRingsMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SauronRingsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SauronRingsMiner terminated\n");
        throw;
    }
}
